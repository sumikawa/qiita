...
AWSは現在利用しているIPアドレスブロックを[JSONファイル](https://docs.aws.amazon.com/ja_jp/general/latest/gr/aws-ip-ranges.html)(```ip-ranges.json```)にして公開していますが、このファイルの過去履歴を知りたいときがあります。
...
```ip-ranges.json``` は変更があるとSNS通知が飛ぶようになっているので(ARNは[ドキュメント](https://docs.aws.amazon.com/ja_jp/general/latest/gr/aws-ip-ranges.html)参照のこと)、この通知をトリガーにして以下のLambdaスクリプト(Python3)を呼び出します。
...
[https://github.com/sumikawa/ip-range](https://github.com/sumikawa/ip-range)
