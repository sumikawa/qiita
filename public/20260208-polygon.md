...
tags:
  ...
  - Polygon
private: false
updated_at: '2026-03-18T15:26:10+09:00'
...
コードを書いたものの、結局、スキャンライン法は使わず。
ピクセルを順番に辿っていくレイトレーシングと同じコードになった。

![output.png](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/13400/d9171d7b-f1cf-4871-a0b3-445438c0fc8b.png)

# ピクセルの座標系と物体の座標系を分ける

ピクセルの座標系と物体の座標系を分けて、前者を```i32```、後者を```f64```にする。ピクセルの座標を物体の座標に変換するときは、```0.5```を加えて、ピクセルの中心点として扱う。
...
構文としてクセがあってまだピンと来ないけど、例外処理をシンプルに記述できるのは分かった。
...
ChatGPTにZバッファー法を教えてもらいつつその4に続く。
